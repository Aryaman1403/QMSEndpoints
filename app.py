...
def on_connect():
    ...
    vitals_queue = get_vitals_queue_kafka()
    socketio.emit('vitals-queue', vitals_queue)

    doctors_queue = get_doctors_queue_kafka()
    socketio.emit('doctors-queue', doctors_queue)

    pharmacy_queue = get_pharmacy_queue_kafka()
    socketio.emit('pharmacy-queue', pharmacy_queue)
    emit('message', {'info': 'WebSocket connected'})
    
KAFKA_BOOTSTRAP_SERVERS = os.environ.get('KAFKA_BOOTSTRAP_SERVERS', 'kafka:9092')

KAFKA_TOPIC = 'vitals-queue'
KAFKA_TOKEN_TOPIC = 'generate-token'
...
KAFKA_VITALS_TOPIC = 'vitals-queue'
KAFKA_DOCTORS_TOPIC = 'doctors-queue'
KAFKA_PHARMACY_TOPIC = 'pharmacy-queue'
...
def get_doctors_queue_kafka():
    today = datetime.now().date()
    # For each room, get list of queued tokens ordered by priority and FIFO (like /api/docrooms/priority_queue)
    all_rooms = DocRoomInfo.query.all()
    result = []
    for room in all_rooms:
        consults = ConsultLog.query.filter_by(roomno=room.roomno, date=today).filter(
...
def get_pharmacy_queue_kafka():
    today = datetime.now().date()
    result = []
    for counter in PharmacyInfo.query.all():
...
def generate_token():
    ...
    vitals_queue = get_vitals_queue_kafka()
    producer.send('vitals-queue', value=vitals_queue)
    producer.flush()
...
def kafka_vitals_consumer_thread():
    ...
    with app.app_context():  # <-- ensures Flask context is active
        for message in consumer:
            app.logger.info(f"[Kafka -> generate-token] Raw message: {message.value}")

            try:
                new_vitals_queue = get_vitals_queue_kafka()
                app.logger.info(f"[Kafka -> vitals-queue] Emitting updated queue: {new_vitals_queue}")

                socketio.emit("vitals-queue", new_vitals_queue)
            except Exception as e:
                app.logger.error(f"Error processing or emitting vitals queue: {e}", exc_info=True)


def kafka_doctors_consumer_thread():
    consumer = KafkaConsumer(
        KAFKA_DOCTORS_TOPIC,
        bootstrap_servers=KAFKA_BOOTSTRAP_SERVERS,
        value_deserializer=lambda m: json.loads(m.decode('utf-8')),
    ...
    with app.app_context():
        for message in consumer:
            socketio.emit('doctors-queue', message.value)

def kafka_pharmacy_consumer_thread():
    consumer = KafkaConsumer(
        KAFKA_PHARMACY_TOPIC,
        bootstrap_servers=KAFKA_BOOTSTRAP_SERVERS,
        value_deserializer=lambda m: json.loads(m.decode('utf-8')),
        auto_offset_reset='latest',
        enable_auto_commit=True
    ...
    with app.app_context():
        for message in consumer:
            socketio.emit('pharmacy-queue', message.value)



threading.Thread(target=kafka_vitals_consumer_thread, daemon=True).start()
threading.Thread(target=kafka_doctors_consumer_thread, daemon=True).start()
threading.Thread(target=kafka_pharmacy_consumer_thread, daemon=True).start()
...
def finished_vitals():
    ...
    doctors_queue = get_doctors_queue_kafka()
    producer.send('doctors-queue', value=doctors_queue)
    producer.flush()
...
def consult_reported():
    ...
    doctors_queue = get_doctors_queue_kafka()
    producer.send(KAFKA_DOCTORS_TOPIC, value=doctors_queue)
    producer.flush()
...
def consult_finished():
    ...
    pharmacy_queue = get_pharmacy_queue_kafka()
    producer.send(KAFKA_PHARMACY_TOPIC, value=pharmacy_queue)
    producer.flush()
